...
final class RevisionedHTTPResponseDecoderTests: ResponseDecoderTests {
    ...
    @Test
    func test_process_returnsNotModifiedResponse_on304() throws {
        try expectProcessToResponse(.notModified, data: .emptyData, statusCode: notModifiedStatusCode)
    }
    ...
    private typealias Response = DecodedResponse<Value>

    private func process(
        data: Data,
        response: HTTPURLResponse
    ) throws -> Response {
        try makeSUT().processRevisioned(data, response)
    }
    ...
    private func expectProcessToResponse(
        ...
        guard case let .response(revisionedResponse) = response else {
            Issue.record("Expected decoded response", sourceLocation: sourceLocation)
            return
        }

        #expect(revisionedResponse.value == expectedValue, sourceLocation: sourceLocation)
        #expect(revisionedResponse.etag == etag, sourceLocation: sourceLocation)
    }
